...
@dp.table(name="catalog_we48.logistics_sdp.silver_staff")
def silver_staff_df():
    df=spark.readStream.table("catalog_we48.logistics_sdp.bronze_staff_data1").select(
...
@dp.table(name="catalog_we48.logistics_sdp.silver_geotag")
@dp.expect("valid_latitude", "latitude > -90 and latitude < 90")
def silver_geo_tag_df():
    df=(spark.readStream.table("catalog_we48.logistics_sdp.bronze_geotag_data").select(
...
@dp.table(name="catalog_we48.logistics_sdp.silver_shipment")
def silver_shipment_df():
    ship_date_col = to_date(col("shipment_date"), "yy-MM-dd") 
    df=(spark.readStream.table("catalog_we48.logistics_sdp.bronze_shipment_data")
